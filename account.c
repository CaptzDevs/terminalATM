...
int selectRow(int min , int max ,char fileName[], selectedTable tableCallBack ){
    char ch ;
    int i = 0;
    int num = 1;
    tableCallBack(fileName,num);
...
Table processTableCSV(const char *filename , int choice) {
    ...
    while (fgets(line, sizeof(line), file) && currentRow <= MAX_ROW) {
        ...
        if(choice == currentRow) printf("\033[1;32m\033[4m");

        printf("%d\t", currentRow); // Print the row number

        if(choice == currentRow) printf("\033[0m");
        ...
        if(choice == currentRow) printf("\033[1;32m");
